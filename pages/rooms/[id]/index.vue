        ...
        <div class="px-4 max-w-1760px mx-auto pt-0 pb-10 lg:py-20 grid grid-cols-2 gap-2">
          ...
          <div class="grid grid-cols-2 gap-2">
            <div v-for="(column, colIdx) in galleryData" :key="colIdx" class="grid gap-2">
              <div v-for="(item, itemIdx) in column" :key="itemIdx" :class="{'rounded-tr-5':colIdx === 1 && itemIdx === 0, 'rounded-br-5':colIdx === 1 && itemIdx === 1}" class="w-full h-74 overflow-hidden">
          ...
          <!-- 訂房須知 -->
           <div class="flex flex-col gap-6">
            ...
            <ul class="list-decimal pl-6 space-y-2">
              <li v-for="(i, idx) in knowList" :key="idx" class="text-3.5 lg:text-4 text-gray-80 text-justify leading-5">{{ i.txt }}</li>
            </ul>
           </div>
         </div>
        <!-- 預約房型卡 -->
            ...
            <div class="mt-7 flex flex-col gap-4">
              ...
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center">
                  ...
                  <button @click="handlePeople(1)" :disabled="people >= 4" class="p-0 rounded-full border-(px solid gray-40) bg-white group">
